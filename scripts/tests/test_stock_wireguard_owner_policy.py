...
class StockWireguardOwnerPolicyTests(unittest.TestCase):
    ...
    def test_reverse_startup_stops_only_stock_compatibility_vpn(self):
        source = ANDROID_SOURCE.read_text(encoding="utf-8")
        ...
        self.assertNotIn("com.example.mobileproxy", body)
        self.assertNotIn("first_party_vpn", body)
    ...
    def test_runtime_owner_contract_excludes_android_vpn_service(self):
        ...
        self.assertNotIn("FirstPartyVpnService", owner_enum)
        self.assertIn('TunnelOwner::parse("first_party_vpn_service").is_err()', config)
