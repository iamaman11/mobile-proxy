#!/usr/bin/env python3
"""Fail-closed offline verifier for the private Android release signing identity.

This canonical public logic validates the four private Android signing inputs without
contacting or mutating the production phone. It decodes the private keystore into an
ephemeral directory, proves the configured alias is readable with `keytool`, signs a
throwaway JAR with `jarsigner` using the configured private-key password, and verifies
the resulting signature before deleting all temporary material.

The verifier never emits the keystore bytes, passwords, alias, certificate details, or
secret-derived values. It does not generate a key, sign a production APK, access ADB,
install/update/uninstall anything, restart services, mutate networking, or access a
provider API.
"""
...
def require_tools() -> None:
    for tool in _REQUIRED_TOOLS:
        require(
            ...
            f"required offline signing verifier tool is missing: {tool}",
        )
...
def verify_release_keystore(canonical_sha: str) -> dict[str, Any]:
    ...
    return {
        ...
        "mode": "offline_release_keystore_verification",
...
def main() -> int:
    args = parse_args()
    try:
        report = verify_release_keystore(args.canonical_sha)
