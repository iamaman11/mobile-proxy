...
_FILESYSTEM_QUARANTINE_CLEANUP_FACT = FactRequirement(
...
ANDROID_FILESYSTEM_QUARANTINE_CLEANUP = OperationContract(
    ...
    steps=(
        ...
        StepContract("post_cleanup_observation", "OBSERVE", "POSTCONDITION"),
        StepContract("accept", "ACCEPT", "POSTCONDITION", acceptance=True),
    ),
    recovery_steps=(
        StepContract("recovery_post_cleanup_observation", "OBSERVE", "RECOVERY"),
        StepContract("recovery_classify", "VERIFY", "RECOVERY", acceptance=True),
    ),
    fact_requirements=(
        _FILESYSTEM_QUARANTINE_CLEANUP_FACT,
