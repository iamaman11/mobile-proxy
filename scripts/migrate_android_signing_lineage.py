...
def sha256_file(path: Path) -> str:
    ...
    require(bool(result) and _SHA256_PATTERN.fullmatch(result[0]) is not None, "APK checksum is invalid")
    return result[0]
...
def package_version(serial: str) -> tuple[int, str]:
    ...
    require(f"Package [{_PACKAGE}]" in result.stdout or f"Package {{{_PACKAGE}" in result.stdout, "production package is not installed")
...
def verify_release_evidence(
    ...
    require(evidence.get("repository") == "iamaman11/mobile-proxy", "Android release evidence repository differs")
    ...
    require(evidence.get("android_functional_source_preserved") is True, "Android functional-source preservation is unproven")
    require(evidence.get("application_id") == _PACKAGE, "Android release package differs")
    require(evidence.get("version_name") == expected_version_name, "Android release versionName differs")
    require(evidence.get("version_code") == expected_version_code, "Android release versionCode differs")
    for field in (
    ...
    require(evidence.get("phone_access_performed") is False, "Android release build evidence unexpectedly used the phone")
    require(evidence.get("phone_mutation_performed") is False, "Android release build evidence unexpectedly mutated the phone")
    digest = evidence.get("artifact_sha256")
    require(isinstance(digest, str) and _SHA256_PATTERN.fullmatch(digest) is not None, "Android release artifact checksum is invalid")
    require(apk.is_file() and apk.stat().st_size > 0, "signed Android release APK is unavailable")
    require(sha256_file(apk) == digest, "signed Android release APK checksum differs")
    return digest


def capture_installed_apk(serial: str, output: Path) -> str:
    ...
    require(output.is_file() and output.stat().st_size > 0, "installed rollback APK was not retained")
...
def restart_runtime_supervisor(serial: str) -> None:
    exact_preflight(serial)
    adb(serial, "shell", "su", "0", "sh", "-c", _supervisor_restart_shell(), timeout=60)
...
def wait_for_local_health(serial: str, *, timeout_seconds: int = 60) -> None:
    ...
    while time.monotonic() < deadline:
        if supervisor_running(serial) and egress_service_running(serial) and local_proxy_ports_ready(serial):
            return
        time.sleep(2)
    raise MigrationFailure("new Android signing generation did not become locally healthy")


def restore_old_generation(serial: str, old_apk: Path, old_code: int, old_name: str) -> bool:
    try:
        if package_present(serial):
            uninstall(serial)
        install(serial, old_apk)
        restored_code, restored_name = package_version(serial)
        require((restored_code, restored_name) == (old_code, old_name), "rollback APK version differs from retained old package")
...
def migrate(args: argparse.Namespace) -> tuple[dict[str, Any], bool]:
    ...
    require(args.authorization == _AUTHORIZATION, "explicit destructive migration authorization is absent")
    serial = require_expected_serial()
    require(shutil.which("adb") is not None, "adb is unavailable")
    require(shutil.which("sha256sum") is not None, "sha256sum is unavailable")
    ...
    require(old_code == args.expected_old_version_code, "installed old Android versionCode differs from approved baseline")
    require(old_name == args.expected_old_version_name, "installed old Android versionName differs from approved baseline")
    old_digest = capture_installed_apk(serial, args.retained_old_apk)
    ...
    try:
        ...
        require(new_code == args.expected_version_code, "installed new Android versionCode differs")
        require(new_name == args.expected_version_name, "installed new Android versionName differs")
        ...
        report["rollback_succeeded"] = restore_old_generation(serial, args.retained_old_apk, old_code, old_name)
        return report, False
...
def main() -> int:
    ...
    try:
        ...
        return 1
    if not accepted:
        print("Android signing-lineage migration failed; bounded rollback was attempted", file=sys.stderr)
        return 1
    return 0
