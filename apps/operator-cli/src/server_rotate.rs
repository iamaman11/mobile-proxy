...
async fn issue_rotation(
    ...
    let request = IssueCommandRequest {
        ...
        deadline_secs: DeadlineWindow::new(args.timeout_secs)
            .context("timeout is outside the supported command deadline range")?,
        idempotency_key: IdempotencyKey::parse(format!("operator-cli-{}", Uuid::new_v4()))
            .expect("generated idempotency key is valid"),
    };
    client
        .post(format!(
            "{}/api/v1/ui/devices/{}/commands",
            args.control_plane_url.trim_end_matches('/'),
            args.device_id
        ))
        .bearer_auth(&args.ui_token)
        ...
        .send()
        .await
        .context("failed to send rotation command to control plane")?
        .error_for_status()
