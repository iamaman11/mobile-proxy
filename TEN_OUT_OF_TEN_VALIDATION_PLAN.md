...
- 2026-06-08 proxy validation completed; see [PROXY_VALIDATION_2026_06_08.md](/home/bose/projects/mobile-proxy/PROXY_VALIDATION_2026_06_08.md): all public proxy modes passed HTTP/HTTPS quality soak `800/800`, parallel HTTPS smoke `120/120`, authentication rejection checks, and target-observed IP privacy checks
- remaining proxy privacy gap: public proxy ingress is raw HTTP/SOCKS TCP, so client-to-VM proxy credentials are not TLS-wrapped; target-side IP privacy is good, but strict untrusted-network privacy requires TLS-protected proxy ingress or a separate encrypted client-to-VM tunnel
